...
export default function PlanScreen() {
  ...
  const handleSave = async () => {
    ...
    triggerObj.channelId = 'income-reminders';

    if (billingCycle !== 'once' || date.getTime() > Date.now()) {
      try {
        notificationId = await Notifications.scheduleNotificationAsync({
          content: {
            title: planCategory === 'income' ? '💰 Money Collection Reminder!' : '📋 Plan Reminder!',
            body: `Time for ${planType}: ${amount} ETB`,
            sound: true,
        ...
        Alert.alert('✅ Reminder Set', `You will be reminded ${billingCycle === 'once' ? `at ${date.toLocaleString()}` : `every ${billingCycle.replace('ly', '')}`}.`);
      } catch (error) {
        console.error("Failed to schedule notification", error);
        Alert.alert('⚠️ Error', 'Failed to schedule the notification.');
      }
    } else {
      // Warn the user and prevent saving with an invalid past time for one-time plans
      Alert.alert('⚠️ Invalid Time', 'Please pick a future date and time for your one-time reminder.');
      return;
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (planToDelete && planToDelete.notificationId) {
      try {
        await Notifications.cancelScheduledNotificationAsync(planToDelete.notificationId);
        console.log(`Cancelled notification for plan ${id}`);
      } catch (err) {
        ...
        >
          <ScrollView style={styles.modalScroll}>
            <View style={styles.modalContent}>
              ...
              <View style={styles.optionsContainer}>
                {['once', 'hourly', 'weekly', 'monthly', 'yearly'].map((cycle) => (
                  <TouchableOpacity
                    key={cycle}
